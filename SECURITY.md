...
Python authoring is a separate execution mode with Unreal Editor's host permissions, enabled by default starting in 1.0.2 at the user's request. It is not covered by the Codex filesystem/network sandbox and can perform arbitrary host operations. Clear the checkbox before submitting for bounded tools only. The visible toggle/banner, local source/result archives and declared-content backups make the mode visible and aid recovery; they do not sandbox code or guarantee complete rollback. ON/OFF selection persists while the panel is open, including reconnects, new threads and Stop turn. Active-turn permission still expires on stop, disconnect and completion. See `THREAT_MODEL.md` and `Docs/USAGE.ja.md`.
