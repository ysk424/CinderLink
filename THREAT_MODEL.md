...
| Arbitrary Editor execution | Python authoring is default-off, visibly indicates host permissions, and is revoked for future calls on stop/disconnect/new thread. No in-process security sandbox is claimed. |
...
- Python authoring is off by default and requires a panel selection plus both edit permissions for the submitted turn. The model cannot enable the mode through a tool. Revocation blocks subsequent bridge calls; it does not undo past execution or stop native code already running.
